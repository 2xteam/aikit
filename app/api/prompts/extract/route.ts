 ...
 *
 * ⚠️ **이 라우트는 아무것도 저장하지 않는다** — 묶음이 아직 없기 때문이다.
 * `/logs/new` 는 [만들기] 를 누를 때 묶음을 만들므로, 분위기를 고르는 시점에는
 * 이미지를 붙일 곳이 없다.
 *
 * 레퍼런스 이미지 **자체는 남긴다** (2026-09-20 사용자 지정). 화면이 파일을 들고
 * 있다가 묶음이 생기면 `role: "reference"` 로 올린다 → app/(app)/logs/new/page.tsx
 *
 * 처음에는 "남의 작품일 수 있으니 저장하지 않는다" 로 두었는데, **입력 사진과
 * 다를 것이 없다** — 그것도 남의 사진일 수 있고 같은 비공개 버킷에 본인만 보게
 * 들어간다. 기록하는 앱에서 "무엇을 보고 만들었는지" 를 잃는 쪽이 더 나쁘다.
 *
